...
exports.renderHead = exports.renderBody = void 0;
/* ---------- Utilities ---------- */
const esc = (v) => v == null
    ? ""
    : String(v)
...
const signatureBlock = (p, s, col) => {
    const n = Math.max(1, Math.min(5, p.lines ?? 1));
    const lines = Array.from({ length: n })
        .map(() => `<div class="border-b" style="border-color:${col.border};height:2rem;"></div>`)
        .join("");
    return `
<section class="${tw("mt-8", s?.wrapper)}">
  <div class="flex flex-col gap-6 w-64">
    ${lines}
    <div class="${tw("text-sm text-slate-600", s?.label)}">${esc(p.label || "")}</div>
  </div>
</section>`;
};
const footerTextBlock = (p, s) => `
<section class="${tw("mt-8 text-center text-sm text-slate-600", s?.text)}">${esc(p.text)}</section>`;
const tableBlock = (p, s, cfg, col) => {
    const title = p.title
        ? `<div class="${tw("mb-2 font-semibold text-slate-800", s?.title)}">${esc(p.title)}</div>`
        : "";
    ...
    const thead = (p.headers || [])
        .map((h) => `<th class="${compact} border-b font-semibold text-left" style="border-color:${col.border}">${esc(h)}</th>`)
        .join("");
    const body = (p.rows || [])
        .map((row, i) => {
        const bg = cfg.table.striped && i % 2 === 1 ? "bg-gray-100" : "bg-white";
        const tds = row
            .map((c) => `<td class="${compact} border-b" style="border-color:${col.border}">${esc(c)}</td>`)
            .join("");
...
<section class="${tw("my-4", s?.wrapper)}">
  ...
  <div class="${tw("tbl-wrap overflow-x-auto", s?.container)}">
    <table class="${tw("tbl w-full border-collapse", cfg.table.border)}" style="border-color:${col.border}">
...
const imageBlock = (p, s) => {
    const sz = [];
    if (p.width)
        sz.push(`width:${p.width};`);
    if (p.height)
        sz.push(`height:${p.height};`);
    const cap = p.caption
        ? `<div class="${tw("text-xs text-slate-500 mt-1 text-center", s?.caption)}">${esc(p.caption)}</div>`
        : "";
    return `
<section class="${tw("my-4", s?.wrapper)}">
...
const renderBody = (r) => {
    ...
    const firstHeader = r.configs.header.visible && r.configs.header.repeat === "first"
    ...
    const parts = r.components
        ...
        switch (c.type) {
            case "header":
                return headerBlock(c.props, c.style);
            case "subheader":
                return subheaderBlock(c.props, c.style);
            case "date":
                return dateBlock(c.props, c.style, r.configs);
            case "para":
                return paraBlock(c.props, c.style);
            case "divider":
                return dividerBlock(r.colors, c.style);
            case "spacer":
                return spacerBlock(c.props, c.style);
            case "pagebreak":
                return pagebreakBlock();
            case "signature":
                return signatureBlock(c.props, c.style, r.colors);
            case "footerText":
                return footerTextBlock(c.props, c.style);
            case "table":
                return tableBlock(c.props, c.style, r.configs, r.colors);
            case "image":
                return imageBlock(c.props, c.style);
            case "image-grid":
                return imageGridBlock(c.props, c.style);
            default:
                return "";
        }
    })
    ...
    const fixedFooter = r.configs.footer.visible
        ? `<footer class="fixed-footer px-4" style="">
         ${r.assets.footerImage
            ...
            : ""}
         <span class="text-sm text-gray-600">${r.configs.footer.text.replace("{{page}}", "").replace("{{pages}}", "")}</span>
       </footer>`
        : "";
    /* background layers using fixed positioning */
    const pageBg = (r.assets.firstPageBackground && r.assets.otherPagesBackground)
        ? `<div class="page-bg-first"></div><div class="page-bg-other"></div>`
        : "";
...
/* First page background - visible only on first page */
.page-bg-first {
  position: fixed;
  top: 0;
  left: 0;
  width: 100vw;
  height: 100vh;
  background-image: url('${r.assets.firstPageBackground || ""}');
...
/* Other pages background - starts from page 2 */
.page-bg-other {
  position: fixed;
  top: 100vh;
  left: 0;
  width: 100vw;
  height: 999vh;
  background-image: url('${r.assets.otherPagesBackground || ""}');
...
@media print{
  ...
  .page-bg-first{position:fixed;}
  .page-bg-other{position:fixed;}
